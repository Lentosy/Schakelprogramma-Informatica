...
\end{itemize}
\section{Functies van de exponenti\"ele orde}
\begin{itemize}[label={}]
 \item{
    \exercise{
            Geef de exponenti\"ele orde van $f(t) = te^{-2t}$
    }{
            \begin{equation*}
             \begin{split}
            ...
            \begin{equation*}
             ...
             \end{split}
            \end{equation*}
            De exponenti\"ele orde is -2.
        }}
 \item{
    \exercise{
        Geef de exponenti\"ele orde van $f(t) = 6e^{3t}$
    }{
        \begin{equation*}
         \begin{split}
         ...
         \end{split}
        \end{equation*}
        Indien $ 3 - \alpha < 0$ dan wordt de limiet 0. De exponenti\"ele orde is dus 3.
    }
...
\begin{itemize}[label={}]
 ...
 \item{
    \exercise{
       ...
       \begin{equation*}
        \begin{split}
            \bigginvlaplace{\frac{3s^3 - 4s^2 + 1}{s^2(s^2 - 2s + 2)}} & = \hbox{splitsen in partieÃ«l breuken} \\  
 ...
 \item{
    \exercise{
       ...
       \begin{equation*}
        \begin{split}
            \bigginvlaplace{\frac{s}{(s^2 + 4)^2}} & = -\frac{1}{4}\bigginvlaplace{\frac{d}{ds}\bigg(\frac{s}{s^2 + 4}\bigg)} \\
                                                   & = -\frac{1}{4}(-1)t\sin 2t \\
                                                   & = \frac{t}{4}\sin 2t
        \end{split}
       \end{equation*}
...
\exercise{
    ...
    \begin{enumerate}
        \item Bepaal de AO 
        \item Bepaal de PO indien $y(\pi/2) = 0$ en $y(\pi) = 1$.
    \end{enumerate}
}{
    Eerst berekenen we de AO
    \begin{equation*}
        \begin{split}
        & \laplace{y'' + y} = \laplace{0} \\
        \Rightarrow\; & s^2Y - sC_1 - C_2 + Y = 0 \\
        \Rightarrow\; & Y = \frac{sC_1}{s^2 + 1} + \frac{C_2}{s^2 + 1} \\
        \Rightarrow\; & y(t) = \bigginvlaplace{\frac{sC_1}{s^2 + 1}} + \bigginvlaplace{\frac{C_2}{s^2 + 1}} \\
        \Rightarrow\; & y(t) = C_1\cos t + C_2\sin t \qquad \hbox{(AO)}
        \end{split}
    \end{equation*}
    Nu berekenen we de PO. We weten dat $y(\pi/2) = 0$ en $y(\pi) = 1$. We krijgen volgend stelsel:
    $$
        \begin{cases}
            0 = C_1\cos \pi/2 + C_2\sin \pi/2 \\
            1 = C_1\cos \pi   + C_2\sin \pi
        \end{cases}
    $$
    waaruit volgt dat $C_1 = -1$ en $C_2 = 0$. De PO wordt $$y(t) = -\cos t$$
}

\exercise{
    Bepaal de PO van $$y'' + 9y = H(t) + \delta(t - 1)$$ met $y(0^+) = 1$, $y'(0^+) = 3$ en $t > 0$.
}{
    \begin{equation*}
        \begin{split}
        & \laplace{y'' + 9y} = \laplace{H(t) + \delta(t - 1)} \\
        \Rightarrow\; & s^2Y - sy(0^+) - y'(0^+) + 9Y = \frac{1}{s} + e^{-s} \\
        \Rightarrow\; & s^2Y - s - 3 + 9Y = \frac{1}{s} + e^{-s} \\
        \Rightarrow\; & Y(s^2 + 9) = \frac{1}{s} + e^{-s} + s + 3 \\
        \Rightarrow\; & Y = \frac{\frac{1}{s} + e^{-s} + s + 3 }{s^2 + 9} \\
        \Rightarrow\; & y = \bigginvlaplace{\frac{\frac{1}{s}}{s^2 + 9}} + \bigginvlaplace{\frac{e^{-s}}{s^2 + 9}} + \bigginvlaplace{\frac{s + 3}{s^2 + 9}}
        \end{split}
    \end{equation*}
    \begin{equation*}
        \begin{split}
        \bigginvlaplace{\frac{\frac{1}{s}}{s^2 + 9}} & = \bigginvlaplace{\frac{1}{9}\cdot\frac{1}{s} - \frac{1}{9}\cdot \frac{s}{s^2 + 9}} \quad \hbox{(splitsen in partieelbreuken)} \\
                                                     & = \frac{1}{9} - \frac{1}{9}\cos 3t
        \end{split}
    \end{equation*}
    \begin{equation*}
        \begin{split}
        \bigginvlaplace{\frac{e^{-s}}{s^2 + 9}} & = f(t - 1)H(t - 1) \quad \hbox{met} \quad f(t) = \bigginvlaplace{\frac{1}{s^2 + 9}} = \frac{1}{3}\sin 3t \\
        & = \frac{1}{3}\sin(3(t - 1))H(t - 1)
        \end{split}
    \end{equation*}
    \begin{equation*}
        \begin{split}
        \bigginvlaplace{\frac{s + 3}{s^2 + 9}} & = \cos 3t + \sin 3t
        \end{split}
    \end{equation*}

    $$y(t) = \frac{1}{9} + \frac{8}{9}\cos 3t + \sin 3t + \frac{1}{3}\sin(3(t - 1))H(t - 1)$$
}
